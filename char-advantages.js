...
export function createAdvantagesSection(ctx) {
  ...
  function renderSide(side) {
    ...
    // If the outer side is collapsed, we hide the list entirely. The
    // caret on .ad-block-header (applied in applySideCollapseUI) is
    // what tells the user something's here. Returning empty keeps the
    // renderer simple — no half-rendered state to worry about.
    if (isSideCollapsed(side)) {
      container.innerHTML = '';
      applySideCollapseUI(side);
      return;
    }
    ...
    CATEGORIES.forEach(c => {
      ...
      const catCollapsed = isCategoryCollapsed(side, c.code);
      const caret = catCollapsed ? '▸' : '▾';
      // Header is the click target. Stop-propagation on any interactive
      // child inside the body isn't needed here because .ad-cat-title is
      // a dedicated element — card clicks are on .ad-cards children.
      html += `<div class="ad-cat-group${catCollapsed ? ' collapsed' : ''}" data-side="${side}" data-cat="${c.code}">`;
      html += `<div class="ad-cat-title ad-collapsible" role="button" tabindex="0"
                    onclick="adToggleCategory('${side}','${c.code}')"
                    onkeydown="if(event.key==='Enter'||event.key===' '){event.preventDefault();adToggleCategory('${side}','${c.code}')}"
                    title="${catCollapsed ? 'Expand' : 'Collapse'} ${c.label}">
        <span class="ad-caret">${caret}</span><span class="ad-cat-title-label">${c.label}</span><span class="ad-cat-count">${items.length}</span>
      </div>`;
  ...
  // Apply the caret + collapsed class on the OUTER side header. Called
  // after each renderSide so the UI stays in sync. The header itself
  // lives in character.html (static markup) so we toggle state on it
  // rather than re-rendering it. Also sets up its click handler on
  // first run, using a data- flag to avoid double-binding.
  function applySideCollapseUI(side) {
    const block = document.getElementById(side + '-list');
    if (!block) return;
    const blockEl = block.closest('.ad-block');
    if (!blockEl) return;
    const header = blockEl.querySelector('.ad-block-header');
    const title  = blockEl.querySelector('.ad-block-title');
    if (!header || !title) return;
    const collapsed = isSideCollapsed(side);

    // Inject the caret once. On re-renders we just update its glyph.
    let caret = title.querySelector('.ad-caret');
    if (!caret) {
    ...
    // Mark the title clickable. Binding once via a data- flag — element
    // lives in static markup, no re-render replaces it.
    if (!title.dataset.adCollapseBound) {
      title.classList.add('ad-collapsible');
      title.setAttribute('role', 'button');
      title.setAttribute('tabindex', '0');
      title.addEventListener('click', () => toggleSide(side));
      title.addEventListener('keydown', (e) => {
        if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); toggleSide(side); }
      });
      title.dataset.adCollapseBound = '1';
    }
    title.title = collapsed ? 'Expand section' : 'Collapse section';

    // Hide/show the list + the add form + the add button. The button
    // should disappear when collapsed — clicking + Add would expand
    // the section anyway, and the button is visual clutter otherwise.
    const list    = blockEl.querySelector(`#${side}-list`);
    const form    = blockEl.querySelector(`#${side}-add-form`);
    const addBtn  = blockEl.querySelector('.ad-add-btn');
    if (list) list.style.display = collapsed ? 'none' : '';
    if (form) form.style.display = collapsed ? 'none' : '';
    if (addBtn && ctx.getCanEdit()) addBtn.style.display = collapsed ? 'none' : 'inline-block';
