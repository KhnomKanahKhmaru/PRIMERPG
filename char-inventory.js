  ...
  function renderContainerEntry(entry, depth, canEdit) {
    ...
    let html = `<div class="inv-entry inv-entry-container${open ? ' open' : ''}${isEditing ? ' editing' : ''}" style="margin-left:${depth * 16}px">
      <div class="inv-entry-head" onclick="invToggleEntry('${escapeHtml(entry.id)}')">
        ...
        ${badge}
        ${canEdit ? `<button class="inv-row-btn inv-row-btn-dur${entry.durabilityTracked ? ' on' : ''}" onclick="event.stopPropagation();invDurabilityToggleTracked('${escapeHtml(entry.id)}')" title="${entry.durabilityTracked ? 'Stop tracking durability' : 'Track durability — reveals a Dur row with hit-instance tracking'}">⛊</button>` : ''}
  ...
  function renderItemEntry(entry, depth, canEdit) {
    ...
    let html = `<div class="inv-entry inv-entry-item${infoOpen ? ' info-open' : ''}${isEditing ? ' editing' : ''}" style="margin-left:${depth * 16}px">
      <div class="inv-entry-head inv-entry-head-item">
        ...
        ${canEdit ? `<button class="inv-row-btn inv-row-btn-dur${entry.durabilityTracked ? ' on' : ''}" onclick="invDurabilityToggleTracked('${escapeHtml(entry.id)}')" title="${entry.durabilityTracked ? 'Stop tracking durability' : 'Track durability — reveals a Dur row with hit-instance tracking'}">⛊</button>` : ''}
